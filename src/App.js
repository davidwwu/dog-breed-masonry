...
class App extends Component {
  ...
  calculateColumnCount = () => {
    this.columnCount = Math.floor(
      this._windowWidth / (this.columnWidth + this.gutterSize)
    );
  };
  ...
  cellRenderer = ({ index, key, parent, style }) => {
    ...
    return (
      <CSSTransition in key={key} timeout={400} classNames="fade">
        <CellMeasurer
          cache={this.cache}
          index={index}
          key={key}
    ...
    return (
      <div className="App">
        <header className="App-header">
          ...
          <h1 className="App-title">Welcome to Dog API List</h1>
        </header>

        <div className="App-intro">
          {this.state.breedsListReady ? (
            <WindowScroller>{this.renderAutoSizer}</WindowScroller>
          ) : (
            "loading..."
          )}
        </div>
